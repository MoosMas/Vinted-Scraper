            ...
            if r.status_code == 200:
                if products:
                    ...
                    try:
                        ...
                        if os.path.exists(vinted_user_path):
                            logging.warning(f"Folder already exists at {vinted_user_path}")
                        else:
                            logging.error(f"Creation of the directory {vinted_user_path} failed: {e}")
                    else:
                        logging.info(f"Successfully created the directory {vinted_user_path}")
...
def download_depop_data(userids):
    ...
    for userid in userids:
        ...
        try:
            ...
            if os.path.exists(depop_user_path):
                print(f"Folder already exists at {depop_user_path}")
            else:
                print(f"Creation of the directory failed: {e}")
                ...
                for x in product_data['videos']:
                    for source in x['outputs']:
                        if source['format'] == 'MP4':
                            ...
                            filepath = depop_product_path + str(file_name)
                            if not args.disable_file_download:
                                if not os.path.isfile(filepath):
